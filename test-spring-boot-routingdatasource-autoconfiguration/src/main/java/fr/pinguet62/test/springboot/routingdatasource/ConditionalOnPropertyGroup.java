...
/** {@link @Conditional} who check if any property start with into {@link Environment}. */
@Retention(RUNTIME)
@Target({ TYPE, METHOD })
...
public @interface ConditionalOnPropertyGroup {
    ...
    static class OnPropertyGroupCondition extends SpringBootCondition {

        private static final Logger LOGGER = getLogger(OnPropertyGroupCondition.class);

        @Override
        public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata) {
            Map<String, Object> conditionalOnPropertyGroup = metadata
                    .getAnnotationAttributes(ConditionalOnPropertyGroup.class.getName());
            ...
            for (String propertyName : properties.keySet())
                if (propertyName.startsWith(propertyNamePattern + "."))
                    return match(forCondition(ConditionalOnPropertyGroup.class)
                            .found("property group \"" + propertyNamePattern + "\" found").items(propertyName));

            return noMatch(forCondition(ConditionalOnPropertyGroup.class)
                    .didNotFind("property group \"" + propertyNamePattern + "\" not found").atAll());
        }
        ...
        private Map<String, Object> getAllProperties(PropertySource<?> propertySource) {
            ...
            if (asList(RandomValuePropertySource.class, StubPropertySource.class, JndiPropertySource.class)
                    .contains(propertySource.getClass()))
                return properties; // not useful
            if (!(propertySource instanceof EnumerablePropertySource)) {
                LOGGER.warn("Skip unsupported {} type: {}", PropertySource.class.getSimpleName(),
